...
def evaluation(models, file_name, dataCenter, threshold_fact, threshold_cond, max_f1, max_std, out_model_name, num_pass, just_eval, write_prediction=False, file_name2=None, just_PR=False, tokenizer=None, LM_model=None, LM_corpus=None, mimo_extractor_fact=None, mimo_extractor_cond=None, pretrain=False, multi_head=None, models_update=[], multi_head_two=None):
	...
	for i in range(num_pass):
		...
		valid_tag_level, valid_Metrics, valid_tuple_level, valid_predicted_str = _evaluation(models, dataCenter, VALID_DATA, threshold_fact, threshold_cond, write_prediction, tokenizer=tokenizer, LM_model=LM_model, LM_corpus=LM_corpus, mimo_extractor_fact=mimo_extractor_fact, mimo_extractor_cond=mimo_extractor_cond, pretrain=pretrain, multi_head=multi_head, multi_head_two=multi_head_two)
		...
		test_tag_level, test_Metrics, test_tuple_level, test_predicted_str = _evaluation(models, dataCenter, TEST_DATA, threshold_fact, threshold_cond, write_prediction, tokenizer=tokenizer, LM_model=LM_model, LM_corpus=LM_corpus, mimo_extractor_fact=mimo_extractor_fact, mimo_extractor_cond=mimo_extractor_cond, pretrain=pretrain, multi_head=multi_head, multi_head_two=multi_head_two)
	...
	print('VALID RSULTs:', valid_tag_levels_mean[-1][-1], valid_tuple_levels_mean[-1][-1])
	...
	if len(TEST_DATA[0]) != 0:
		...
		print('TEST RSULTs:', test_tag_levels_mean[-1][-1], test_tuple_levels_mean[-1][-1])
...
def _core_evaluation(OUTs, SENTENCEs, instance_list, predict_fact_batch, predict_condition_batch, predict_fact_tuples_batch, predict_cond_tuples_batch, dataCenter, threshold_fact, threshold_cond, write_prediction=False, is_filter=False):
	...
	for i in range(len(predict_fact_batch)):
		...
		p_f, r_f, ps_f, rs_f = match_score(_facts, facts, 'f')
		p_c, r_c, ps_c, rs_c = match_score(_conditions, conditions, 'c')
...
def _evaluation(models, dataCenter, data, threshold_fact, threshold_cond, write_prediction=False, device=torch.device("cuda"), tokenizer=None, LM_model=None, LM_corpus=None, mimo_extractor_fact=None, mimo_extractor_cond=None, pretrain=False, multi_head=None, multi_head_two=None):
	...
	for index in range(batches):
		...
		predict_fact_batch, predict_condition_batch, hidden_out_batch = Tag_Extractor(SENTENCEs_batch, POSTAGs_batch, CAPs_batch, models, tokenizer, LM_model, LM_corpus, device, multi_head, multi_head_two)
...
def Tag_Extractor(SENTENCEs, POSTAGs, CAPs, models, tokenizer, LM_model, LM_corpus, device, multi_head, multi_head_two=None):
	...
	if multi_head != None:
		predict_fact_batch, predict_condition_batch, hidden_out_batch = multi_head(hiddens)
	else:
		assert len(models) == 1
...
def apply_model(models, batch_size, dataCenter, device, weight_classes_fact=None, weight_classes_condition=None, LM_model=None, LM_corpus=None, tokenizer=None, mimo_extractor_fact=None, mimo_extractor_cond=None, pretrain=False, multi_head=None, multi_head_two=None, is_semi=False, eval_pack=[]):
	...
	if multi_head == None:
		...
		if len(models) != 3:
			for param in multi_head.parameters():
				if param.requires_grad:
					params.append(param)
			models_update.append(multi_head)
		else:
			for param in multi_head_two.parameters():
				if param.requires_grad:
					params.append(param)
			models_update.append(multi_head_two)
	...
	for index in range(batches):
		...
		if is_semi:
			pre_max_f1 = max_f1
			max_f1, max_std = evaluation(models, out_file, dataCenter, 0, 0, max_f1, max_std, out_model_name, num_pass, False, write_prediction=True, file_name2='./predicts/'+out_file.split('/')[-1], LM_model=LM_model, just_PR=False, LM_corpus=LM_corpus, tokenizer=tokenizer, mimo_extractor_fact=mimo_extractor_fact, mimo_extractor_cond=mimo_extractor_cond, pretrain=pretrain, multi_head=multi_head, models_update=models_update, multi_head_two=multi_head_two)
...
def auto_labeling(models, dataCenter, device, data_file, AR, TC, TCDEL, SH, DEL, LM_model=None, LM_corpus=None, tokenizer=None, mimo_extractor_fact=None, mimo_extractor_cond=None, pretrain=False, multi_head=None, multi_head_two=None):
